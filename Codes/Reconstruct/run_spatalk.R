...
obj <- dec_celltype(obj, sc_data = (as.matrix(sc_counts)), as.character(sc_celltypes$celltype), use_n_cores = 20,iter_num = 200, if_doParallel = T,dec_result = convs)
...
write.csv(genes ,paste0(out_f,'/spatalk/genes.csv',sep = "",collapse = NULL),row.names = FALSE)
write.csv(cells ,paste0(out_f,'/spatalk/cells.csv',sep = "",collapse = NULL),row.names = FALSE)
...
write.csv(dense_matrix ,paste0(out_f,'/spatalk/exp.csv',sep = "",collapse = NULL),row.names = FALSE)
